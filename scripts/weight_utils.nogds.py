...
def fastsafetensors_weights_iterator(
    ...
    nogds = pg.size() > 1
    # qwen38-flash-dgx: this box has no GPU Direct Storage (gdscheck -p reports
    # every backend Unsupported, no /dev/nvidia-fs*). With nogds=False the
    # library retries GDS on every shard (~75 s/shard); with nogds=True the same
    # files load at ~2.75 GiB/s cold. The RuntimeError fallback below never
    # fires here because the library swallows the failure per shard.
    if os.environ.get("VLLM_FASTSAFETENSORS_NOGDS", "0") == "1":
        nogds = True
    # Files the weight loader does not need at all. fastsafetensors moves whole
    # files to the device, so the 10 model-plefp8-* shards (47.7 GiB, read by
    # the PLE mmap loader itself when VLLM_PLE_MMAP=1) would otherwise be
    # streamed through device buffers on every pass, main model and MTP draft.
    # vLLM walks this iterator once per model in the process: pass 1 is the main
    # model, pass 2 the MTP draft. The draft's tensors live in three bf16 shards,
    # but fastsafetensors moves whole files, so pass 2 re-streamed all 75 GiB
    # through device staging on top of the loaded weights and the kernel
    # OOM-killed the container (2026-09-05, 19:57, Docker OOMKilled=true).
    # VLLM_FASTSAFETENSORS_PASS2_GLOB restricts later passes to matching files.
    global _FAST_PASS
    _FAST_PASS = globals().get("_FAST_PASS", 0) + 1
    _pass2 = os.environ.get("VLLM_FASTSAFETENSORS_PASS2_GLOB", "")
    if _FAST_PASS >= 2 and _pass2:
        import fnmatch as _fn
        _b = len(hf_weights_files)
        hf_weights_files = [f for f in hf_weights_files
                            if any(_fn.fnmatch(os.path.basename(f), p) for p in _pass2.split(",") if p)]
        logger.info_once("fastsafetensors: pass %d keeps %d of %d files matching %s",
                         _FAST_PASS, len(hf_weights_files), _b, _pass2)
    _skip = os.environ.get("VLLM_FASTSAFETENSORS_SKIP_GLOB", "")
    if _skip:
        import fnmatch
        _pats = [p for p in _skip.split(",") if p]
        _before = len(hf_weights_files)
        hf_weights_files = [
            f for f in hf_weights_files
            if not any(fnmatch.fnmatch(os.path.basename(f), p) for p in _pats)
        ]
        logger.info_once(
            "fastsafetensors: skipping %d of %d files matching %s",
            _before - len(hf_weights_files), _before, _skip,
        )
    ...
    # Unified memory (GB10): the page cache these reads fill is not reclaimed
    # for CUDA allocations (measured 2026-09-05: cudaMalloc fails with MemFree
    # ~0.5 GiB while tens of GiB sit in cache). Drop each file's clean pages
    # behind the reader once a second. fadvise needs no privilege.
    def _evict_all() -> None:
    ...
    def _evictor() -> None:
        while not _evict_stop.wait(1.0):
            _evict_all()

    _evict_thread = None
    if os.environ.get("VLLM_FASTSAFETENSORS_EVICT", "1") == "1":
        _evict_thread = threading.Thread(target=_evictor, daemon=True)
        _evict_thread.start()
    ...
    try:
        ...
        if _evict_thread is not None:
            _evict_thread.join(timeout=5)
            _evict_all()
        if pl is not None:
            pl.close()
        # The per-file device staging buffers were torch allocations; hand the
        # reserved-but-free blocks back to the driver so vLLM's memory profile
        # sees the weights, not the weights plus the staging high-water mark.
        try:
            torch.cuda.synchronize(device)
            torch.cuda.empty_cache()
        except Exception:  # noqa: BLE001
            pass
        # The pinned staging buffers go back to PyTorch's *host* caching
        # allocator, which keeps them forever. On unified memory (GB10) that
        # is 14.8 GiB of Shmem the engine can no longer use (measured
        # 2026-09-05: "Available KV cache memory: -0.02 GiB"). Release it.
        for _fn in ("_host_emptyCache",):
            _f = getattr(torch._C, _fn, None)
            if _f is not None:
                try:
                    _f()
                    logger.info_once("fastsafetensors: released pinned host cache")
                except Exception:  # noqa: BLE001
                    pass
